...
cleanup_symlinks() {
    ...
    if [ -n "$active_path" ] && [ -d "$active_path" ]; then
        ...
        # Iterate over the game directory and remove corresponding symlinks in RetroArch
    ...
    # Always clean up the retroarch.exe symlink just to be safe
...
delete_saved_game() {
    ...
    if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -gt 0 ] && [ "$choice" -le "${#saved_games[@]}" ]; index=$((choice-1)); then
        ...
        # Escape string for sed and remove
...
if [ -z "$RA_PATH" ] || [ ! -d "$RA_PATH" ]; then
    ...
    while true; do
        ...
        if [ -d "$RA_PATH" ]; then
            ...
            log "RetroArch path set to: $RA_PATH"
            echo -e "${GREEN}✔ RetroArch path saved!${NC}\n"
            break
        else
